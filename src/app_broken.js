...
const S={l:!1,a:[],w:JSON.parse(localStorage.getItem('animeverse_watchlist')||'[]'),e:{},c:new Map(),q:[],ep:null,p:null,s:!1,provider:'gogoanime'};
const A={u:'/api',cu:'https://api.consumet.org',r:3,t:3e4,o:1e4};
...
try{
    const d=await apiRequest(`/search?q=${encodeURIComponent(q)}`);
    const arr=(d&&d.results)||[];
    return arr.map(x=>({id:x.id,provider:x.provider||S.provider,title:x.title,englishTitle:x.english_title||x.title,type:'',episodes:null,year:x.releaseDate||'Unknown',score:'N/A',synopsis:'',genres:[],rating:'',image:x.image||generatePlaceholderImage(x.title),url:x.url||'#'}));
}catch(e){return getFallbackData(q)}
}

async function loadFeaturedAnime(){
try{showLoading('Loading...');const d=await apiRequest('/trending');displayResults(processApiData(d.results||[]),'featured')}
catch(e){showToast('Failed to load','error')}finally{hideLoading()}
}

async function loadTrendingAnime(){
try{showLoading('Loading...');const d=await apiRequest('/trending');displayResults(processApiData(d.results||[]),'trending')}
catch(e){showToast('Failed to load','error')}finally{hideLoading()}
}
...
return arr.map(a=>({id:a.id,provider:a.provider||S.provider,title:a.title,englishTitle:a.english_title||a.title,type:'',episodes:null,year:a.releaseDate||'Unknown',score:'N/A',synopsis:'',genres:[],rating:'',image:a.image||generatePlaceholderImage(a.title),url:a.url||'#'}))
}
...
async function showAnimeDetails(a){
try{
    showLoading('Loading details...');
    // Fetch detailed info from backend if episodes or synopsis missing
    let info=a;
    if(!a.episodes||!a.synopsis||!a.genres||a.genres.length===0){
        const provider=a.provider||S.provider; const id=a.id;
        const d=await apiRequest(`/anime/${provider}/${encodeURIComponent(id)}`);
        if(d){
            info={...a,title:d.title||a.title,englishTitle:d.english_title||a.englishTitle,synopsis:d.synopsis||'',genres:d.genres||[],episodes:d.totalEpisodes||d.episodes||a.episodes,image:d.image||a.image,provider:provider,episodes_list:d.episodes_list||[]};
        }
    }
    const g=(info.genres||[]).map(genre=>`<span class=\"inline-block bg-anime-accent/20 text-anime-accent px-3 py-1 rounded-full text-sm\">${genre}</span>`).join('');
    const epButtons=(info.episodes_list||[]).slice(0,50).map(ep=>`<button class=\"px-3 py-2 rounded bg-anime-secondary hover:bg-gray-700 border border-gray-700 text-sm\" onclick=\"playEpisode('${info.provider||S.provider}','${ep.id||ep.episodeId||ep.url||''}',${ep.number||1})\">EP ${ep.number||1}</button>`).join(' ');
    const content=`<div class=\"flex flex-col lg:flex-row gap-6\"><img src=\"${info.image}\" alt=\"${info.title}\" class=\"w-full lg:w-80 h-auto object-cover rounded-lg\"><div class=\"flex-1\"><h2 class=\"text-3xl font-bold text-anime-text mb-4\">${info.title}</h2><div class=\"grid grid-cols-2 gap-4 mb-4\"><div><span class=\"text-anime-muted\">Type:</span><span class=\"text-anime-text ml-2\">${info.type||'Anime'}</span></div><div><span class=\"text-anime-muted\">Year:</span><span class=\"text-anime-text ml-2\">${info.year||'Unknown'}</span></div><div><span class=\"text-anime-muted\">Episodes:</span><span class=\"text-anime-text ml-2\">${info.episodes||'Unknown'}</span></div></div><div class=\"mb-4\"><h3 class=\"text-anime-text font-semibold mb-2\">Genres:</h3><div class=\"flex flex-wrap gap-2\">${g}</div></div><div class=\"flex gap-3 mb-4\"><button class=\"bg-anime-accent hover:bg-anime-accent-hover text-white px-6 py-3 rounded-lg transition-colors flex items-center\" onclick=\"(function(){const list=document.getElementById('episode-list'); if(list) list.scrollIntoView({behavior:'smooth'});})()\"><i class=\"fas fa-play mr-2\"></i>Watch</button><button class=\"bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded-lg transition-colors flex items-center\" onclick=\"toggleWatchlist(${JSON.stringify(a).replace(/\\\"/g,'&quot;')})\"><i class=\"fas fa-bookmark mr-2\"></i>${S.w.some(w=>w.id===a.id)?'Remove':'Add to Watchlist'}</button></div></div></div><div class=\"mt-6\"><h3 class=\"text-anime-text font-semibold mb-3\">Synopsis</h3><p class=\"text-anime-muted leading-relaxed\">${info.synopsis||'No synopsis available'}</p></div><div id=\"episode-list\" class=\"mt-6\"><h3 class=\"text-anime-text font-semibold mb-3\">Episodes</h3><div class=\"flex flex-wrap gap-2\">${epButtons||'<span class=\"text-anime-muted\">No episodes available</span>'}</div></div>`;
...
async function playEpisode(provider,episodeId,epNum){
 try{showLoading(`Fetching streams for EP ${epNum||''}...`);
 const d=await apiRequest(`/watch/${provider}/${encodeURIComponent(episodeId)}`);
 const sources=(d&&d.sources)||[];
 if(!sources.length){showToast('No sources found','error');return}
 const hls= sources.find(s=>s.isM3U8)|| sources.find(s=>/m3u8/i.test(s.quality||''))||sources[0];
 const video=document.getElementById('player-video');
 const modal=document.getElementById('player-modal');
 if(!video||!modal){showToast('Player not ready','error');return}
 // Reset
 video.pause(); video.removeAttribute('src'); video.load();
 if(window.Hls && window.Hls.isSupported() && hls && hls.url && /m3u8/.test(hls.url)){
   if(S.p) { S.p.destroy(); S.p=null; }
   S.p=new Hls();
   S.p.loadSource(hls.url);
   S.p.attachMedia(video);
   S.p.on(Hls.Events.MANIFEST_PARSED,()=>{ video.play().catch(()=>{}); });
 } else {
   video.src=hls.url; video.play().catch(()=>{});
 }
 modal.classList.remove('hidden'); S.s=true; S.ep=episodeId;
 }catch(e){showToast('Failed to start playback','error')}finally{hideLoading()}
}

function closePlayer(){
 const modal=document.getElementById('player-modal'); const video=document.getElementById('player-video');
 if(S.p){ try{S.p.destroy()}catch(_){} S.p=null; }
 if(video){ try{video.pause()}catch(_){} }
 modal?.classList.add('hidden'); S.s=false;
}
